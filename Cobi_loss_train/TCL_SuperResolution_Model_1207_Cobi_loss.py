...
class TCL_SuperResolution(object):
    ...
    def Choose_Model(self, Model_index):
        ...
        self.model = net(num_channels=self.num_channels, scale_factor=self.scale_factor, d=56, s=12, m=1)
    ...
    def validation(self, epoch, val_data_loader): #input as YCbCr   to be complete
        ...
        for _, (LR, target) in enumerate(val_data_loader):
            for ch in range(3):
                ...
                print("validation dataset recon_img.size()")
                print(recon_imgs.size())
                if ch == 0:
    ...
    def test(self):
        ...
        for ind_image, (LR, target) in enumerate(test_data_loader):
            ...
            ## groundTruth
            # gt_img = utils.shave(target[1], border_size=2 * self.scale_factor)
            #
            # gt_img *= 255.0
