...
class CSFlow:
    ...
    def create(I_features, T_features, distance, nnsigma=float(1.0), b=float(1.0)):
        ...
        features_grid = features_grid.repeat(N, 1, 1, 1).cuda() #TODO cpu and cuda version
...
def CX_loss(T_features, I_features, distance=Distance.L2, nnsigma=float(1.0), w_spatial=0.1):
    ...
    cs_flow, cs_flow_sp = CSFlow.create(I_features_tf, T_features_tf, distance, nnsigma)
    # sum_normalize:
    # To:
    cs = cs_flow.cs_NHWC
    ...
    k_arg_max_NC = torch.argmax(torch.reshape(cs_comb, [cs_comb.shape[0], -1, cs_comb.shape[3]]), dim=1)
